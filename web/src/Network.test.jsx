...
jest.mock("./NetworkWiredStatus.jsx", () => () => "Summary for active wired connections");
jest.mock("./NetworkWifiStatus.jsx", () => () => "Summary for active WiFi connections");
jest.mock("./client");

const networkMock = {
  activeConnections: () => Promise.resolve([]),
  listen: jest.fn()
};
...
it("renders wired connections status", async () => {
  installerRender(<Network />);
  await screen.findByText("Summary for active wired connections");
});

it("renders wifi connections status", async () => {
  installerRender(<Network />);
  await screen.findByText("Summary for active WiFi connections");
});
...
//       "Wired connected: active-wired-connection 10.0.0.1/22 - another-wired-conn 10.0.0.4/22"
//     );
//   });
// });
